...
describe("session backfill gateway methods", () => {
  ...
  it("validates preview params and returns at most three samples per day", async () => {
    ...
    expect(executeBatchMock.mock.calls[0]?.[0]).not.toHaveProperty("workspaceDir");
  ...
  it("rejects a shared workspace before executing backfill when another owner has cut over", async () => {
    isLegacyMemorySurfaceDisabled.mockImplementation((agentId: string) => agentId === "alpha");
    const { methods } = createHarness({
      agents: {
        list: [
          { id: "alpha", workspace: "/tmp/shared-workspace" },
